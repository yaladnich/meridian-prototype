...
(function () {
  ...
  function build(box) {
    ...
    // внутрішнє кільце (конус). coneW — нерухома обгортка для маски перекриття й сортування, cone рухається всередині.
    // Зовнішні поверхні малюються знизу вгору (так вони перекривають одна одну правильно), стінка отвору — лише в межах
    // верхнього отвору, наскрізний просвіт (перетин верхнього й нижнього отворів) лишається порожнім.
    const coneW = el('g', {}, scn), cone = el('g', {}, coneW);
    const lens = el('clipPath', { id: 'brl-' + id }, defs);
    el('path', { d: 'M-3000 -3000H3000V3000H-3000Z' + poly(clipConvex(loop(17.5, 16.2), loop(17.5, .8)), true), 'clip-rule': 'evenodd' }, lens);
    const top = el('clipPath', { id: 'bro-' + id }, defs);
    el('path', { d: ring(18.3, 17) }, top);
    const cb = el('g', { 'clip-path': 'url(#brl-' + id + ')' }, cone);
    rev(cb, 25.4, 0, 26, .6, 1);
    const ribW = rev(cb, 26, .6, 25.92, 2.18, 1);
    const ledge = rev(cb, 25.92, 2.18, 23.5, 1.58, -1);
    const rw = rev(cb, 23.5, 1.58, 20.67, 15.17, 1);
    const srw = rev(cb, 21.95, 15.45, 21.95, 16.5, 1);
    const cTop = rev(cb, 21.95, 16.5, 21.45, 17, 1);
    ann(cb, 18.3, 21.45, 17);
    const ci = el('g', { 'clip-path': 'url(#bro-' + id + ')' }, cb);
    const bore = rev(ci, 17.5, 16.2, 17.5, .8, -1);
    const bch = rev(ci, 18.3, 17, 17.5, 16.2, -1);
    arcLine(ci, 17.5, 16.2, bch, true); arcLine(ci, 17.5, .8, bore, true);
    line(cb, ring(21.45, 17)); line(cb, ring(18.3, 17));
    arcLine(cb, 21.95, 16.5, cTop, true); arcLine(cb, 21.95, 15.45, srw); arcLine(cb, 20.67, 15.17, rw, true);
    arcLine(cb, 23.5, 1.58, rw, true); arcLine(cb, 25.92, 2.18, ledge, true);
    arcLine(cb, 26, .6, ribW, true); arcLine(cb, 25.4, 0, [Math.PI, 2 * Math.PI]);
    for (const s of [-1, 1]) {
      sil(cb, [P(s * 25.4, 0, 0), P(s * 26, 0, .6), P(s * 25.92, 0, 2.18)]);
      sil(cb, [P(s * 23.5, 0, 1.58), P(s * 20.67, 0, 15.17)]);
      sil(cb, [P(s * 21.95, 0, 15.45), P(s * 21.95, 0, 16.5), P(s * 21.45, 0, 17)]);
    }
    ...
    return { box, svg, scn, crect, lines, fills, cupB, cupF, cone, coneW, cageSec, bridges, rolls, vis, order: [] };
  }
